    ...
    window.customElements.define('zero-md', class extends HTMLElement {
      ...
      _buildStyles() {
        return new Promise(resolve => {
          // Instead of building styles, we can just resolve an empty string.
          // You can still fetch CSS from URLs if needed, but don't insert them.
          let cssPromises = [];
          
          // Next try reading from css-urls
          if (Array.isArray(this.cssUrls) && this.cssUrls.length) {
            cssPromises = this.cssUrls.map(url => this._getStylesheet(url));
          }
  
          Promise.all(cssPromises).then(data => {
            console.log('Loaded styles:', data); // You can log or process the styles without inserting them.
            resolve(''); // Return an empty string
          }).catch(() => resolve('')); // Fallback to empty string if there's an error
        });
      }
